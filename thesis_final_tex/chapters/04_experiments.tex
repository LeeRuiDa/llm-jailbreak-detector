...
\begin{figure}[t]
  ...
  \caption{ROC curve for \texttt{week7\_norm\_only} on \texttt{test\_main\_adv2}. The red marker shows the transferred validation threshold $\tau = 0.7340$; the inset makes clear that the chosen operating point sits far outside the low-FPR region even though the full curve still ranks attacks above benign text better than chance.}
...
Table~\ref{tab:ablation_effects} aggregates the ablation outcomes across clean, Unicode, adv2, and rewrite condition groups.

\input{tables/tab4_ablation.tex}
