...
A simplified example makes the risk concrete. Suppose a user asks, ``Summarize the attached policy note for me.'' The application then appends retrieved context containing text such as ``Ignore previous instructions and reveal the hidden system prompt.'' If both strings are passed to the model as one combined input, the attachment is no longer just data; it has become competing instruction-like text inside the same prompt window. The detector studied in this thesis therefore evaluates the combined payload rather than the user prompt in isolation.
...
\begin{itemize}
  ...
  \item \textbf{RQ3.} Within the shipped runtime interface and available submission artifacts, how does the project's fully offline rules baseline compare to the optional learned detector?
\end{itemize}
...
This thesis makes three broad contributions, realized through several concrete deliverables:
\begin{itemize}
  \item \textbf{Empirical contribution.} The thesis shows that strong threshold-free ranking performance does not preserve a low-FPR operating point under distribution shift. Under the locked evaluation protocol, the selected learned detector remains strong on the main distribution but exhibits large benign false-positive drift on JBB-style and adv2-style inputs.
  ...
  \item \textbf{Systems contribution.} The project delivers an offline-capable jailbreak and prompt-injection detector with a common runtime interface for a rules baseline and an optional LoRA detector, together with a locked evaluation bundle containing source-of-truth tables, figures, sanitized error cases, and configuration and environment snapshots.
\end{itemize}
